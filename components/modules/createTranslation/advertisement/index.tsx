    ...
    return (<div key={key} className={clsx(s.rowContainer, checked && s.checkedItem)}>
        ...
        <div className={s.inputsContainer}>
            <div className={s.datePickersContainer}>
                <input type={'date'} className={clsx("input", s.datePicker)}/>
                <input type={'date'} className={clsx("input", s.datePicker)}/>
            </div>
            {advertisement.isGenre && (
                <div>
                    <label htmlFor={"genreSelect"} className={s.selectLabel}>Оберіть жанр</label>
                    <select id={"genreSelect"} className={clsx("select", s.customSelect)}></select>
                </div>
            )}
        </div>
        <div className={s.orderButtonContainer}>
            <button className={clsx('button', s.orderButton, checked && s.checkedButton)}>{successIcon()}Замовити
            </button>
        </div>
    </div>)
}
...
const Advertisement = ({advertisements}: AdvertisementProps) => {
    ...
    return (<>
        <h4 className={s.alarmText}>{`* Увага, після натискання на кнопку "Замовити" вартість реклами автоматично списується з вашого балансу`}</h4>
